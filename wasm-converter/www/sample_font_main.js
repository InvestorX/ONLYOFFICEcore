...
// main.jsをこのファイルに差し替えると、下記のサンプルフォントが自動で読み込まれます。
// 配列を書き換えるだけで、任意の外部フォントを簡単に追加できます。
const SAMPLE_FONTS = [
    {
        name: 'Roboto-Regular',
        url: 'https://cdn.jsdelivr.net/npm/@fontsource/roboto/files/roboto-latin-400-normal.woff2',
        note: '英数字用サンプル（woff2, 約21KB）'
    },
    {
...
// サンプルフォント読み込み処理
async function preloadSampleFonts() {
    if (!converter) return;

    showStatus('サンプルフォントを読み込み中...', 'info');
    for (const font of SAMPLE_FONTS) {
        try {
            console.log(`[sample-font] downloading ${font.name} from ${font.url}`);
            const resp = await fetch(font.url);
            if (!resp.ok) throw new Error(`HTTP ${resp.status}`);
            const buffer = await resp.arrayBuffer();
            const data = new Uint8Array(buffer);
            converter.addFont(font.name, data);
            console.log(`[sample-font] loaded ${font.name} (${formatFileSize(data.length)}) ${font.note ? `- ${font.note}` : ''}`);
        } catch (e) {
            console.warn(`[sample-font] ${font.name} の読み込みに失敗しました:`, e);
            showStatus(`サンプルフォント「${font.name}」の取得に失敗しました: ${e}`, 'error');
        }
    }
    updateFontList();
    showStatus('サンプルフォントの読み込みが完了しました。配列を書き換えて好きなフォントを追加できます。', 'success');
}
